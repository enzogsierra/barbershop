...
async function onServiceAdded(e)
{
    ...
    const response = await sendForm(form, "new");
    ...
    if(response.response === 200)
    {
        ...
        tr.setAttribute("service-id", `${response.id}`);
        tr.innerHTML = 
        `
            <tr service-id="${response.id}">
...
function onServicesClick(e)
{
    ...
    if(btn.id == "service-btn-edit")
    {
        ...
        save.onclick = async function(e)
        {
            ...
            const response = await sendForm(form, "edit");
            
            /*const options = {method: "POST", body: form};
            const send = await fetch(`http://${window.location.host}/admin/services/edit`, options);
            const response = await send.json();*/

            if(response.response == 200) // Servicio editado correctamente
            {
                // Re-generar html
    ...
    if(btn.id === "service-btn-delete")
    {
        ...
        .then(async function(result) 
        {
            if(result.isConfirmed) 
            {
                ...
                const response = await sendForm(form, "delete");

                /*const options = {method: "POST", body: form};
                const send = await fetch(`http://${window.location.host}/admin/services/delete`, options);
                const response = await send.json();*/
